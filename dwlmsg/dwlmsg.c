...
dwl_ipc_output_frame(void *data, struct zdwl_ipc_output_v2 *dwl_ipc_output)
{
    ...
    /* Replace this block inside dwl_ipc_output_frame(): */
    ...
    if (tflag) {
      ...
      /* Parse initial tag number as 1-based instead of 0-based */
      for (; *t && *t >= '0' && *t <= '9'; t++)
        i = *t - '0' + i * 10;

      /* Convert user-facing tag number to internal zero-based index */
      if (i > 0)
        i--;
