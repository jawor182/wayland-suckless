...
togglescratch(const Arg *arg)
{
	...
	if (found) {
		if (VISIBLEON(c, selmon)) {
			...
			/* move scratchpad to the target monitor */
			setmon(c, selmon, 0);

			/* show it on the current tag */
			c->tags = selmon->tagset[selmon->seltags];

			/* center it exactly like movecenter */
			_movecenter(c, 0);
...
updatebar(Monitor *m)
{
  ...
  m->b.real_height = (int)ceilf((float)m->b.height / m->wlr_output->scale);
}
